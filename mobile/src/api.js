// API BASE configurations. Uses your local network IP (default: http://192.168.100.4:8000/api)
// Can be overridden by setting EXPO_PUBLIC_API_URL in Expo.
export const API_BASE = (process.env.EXPO_PUBLIC_API_URL || 'http://192.168.100.4:8000/api').replace(/\/$/, '');
...
export function buildProductFormData(raw) {
  ...
  if (raw.image && raw.image.uri) {
    fd.append('image', {
      uri: raw.image.uri,
      name: raw.image.name || 'photo.jpg',
      type: raw.image.type || 'image/jpeg',
    });
  }
  return fd;
}
